   Solidity Group Project

~Project Overview

This repository contains our group work for the Solidity course assignment. 
The project demonstrates our progress in learning smart contract development, including code implementation, course materials, and supporting documentation.

 Group Members                      id

Hemen Girma                    ugr/30650/15

Kalkidan Wondwosen             ugr/30746/15

Martha Alemayehu               ugr/30856/15

Bethlehem Kefeni               ugr/30289/15

Hanamariam Mesfin              ugr/25483/15

~Course Information

 Platform: Alchemy University

 Topic: Solidity & Smart Contract Development

 Focus Areas:
  - Basics of Solidity
  - Writing Smart Contracts
  - Deployment Concepts
  - Blockchain Fundamentals

      ~ Repository Structure

/code                  # Solidity source files

/screenshots  #Progressandcompletionscreenshots

/course-materials    #Notesandlearningresources

/docs                # Additional documentation

   ~ What We Learned

- How blockchain works and its real-world applications
- Writing and structuring Solidity smart contracts
- Basics of decentralized applications (DApps)
- Importance of secure and efficient code


  ~Evidence of Work 

Screenshots and code files in this repository demonstrate our learning progress and course completion.

~Tools & Technologies

- Solidity
- GitHub
- Alchemy University Platform
